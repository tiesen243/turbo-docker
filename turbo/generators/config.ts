    ...
    prompts: [
      {
        ...
        name: 'packageName',
        message:
          'What is the name of the package? (You can skip the `@turbo-docker/` prefix)',
      },
      {
        type: 'input',
        name: 'deps',
    ...
    actions: [
      (answers) => {
        if (
          ...
          if (answers.packageName.startsWith('@turbo-docker/'))
            answers.packageName = answers.packageName.replace('@turbo-docker/', '')
        }
